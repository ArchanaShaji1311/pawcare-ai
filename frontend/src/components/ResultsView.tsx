...
const urgencyStyle: Record<string, { bg: string; text: string; label: string }> = {
  urgent: { bg: "bg-red-50 border-red-200", text: "text-red-700", label: "See a vet urgently" },
  soon: { bg: "bg-orange-50 border-orange-200", text: "text-orange-700", label: "Schedule a vet visit soon" },
  routine: { bg: "bg-amber-50 border-amber-200", text: "text-amber-700", label: "Mention at your next vet visit" },
  none: { bg: "bg-emerald-50 border-emerald-200", text: "text-emerald-700", label: "No urgent concerns" },
};
...
function ConditionCard({ c }: { c: DetectedCondition }) {
  return (
    ...
    >
      ...
      <p className="text-sm text-slate-600 mt-2">{c.explanation}</p>
    ...
    >
      <div className="rounded-3xl bg-white border border-slate-100 shadow-xl shadow-slate-900/5 p-6 sm:p-8">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            {data.breed && (
              <p className="text-sm text-slate-500 mt-0.5">Breed: {data.breed}</p>
            )}
          </div>
          <div className="text-right">
            <p className="text-xs text-slate-400">Overall confidence</p>
            <p className="text-2xl font-extrabold text-brand-600">
              {Math.round(data.overall_confidence * 100)}%
            </p>
          </div>
        </div>
        <p className="text-slate-600 mt-3">{data.overall_summary}</p>
      </div>
      ...
      <div className={`rounded-2xl border p-4 flex items-start gap-3 ${urgency.bg}`}>
        ...
        <div>
          ...
          {data.vet_alert.reasons.length > 0 && (
            <ul className="mt-1 text-sm text-slate-600 list-disc list-inside space-y-0.5">
      ...
      <div>
        ...
        {data.conditions.length === 0 ? (
          <div className="rounded-2xl border border-emerald-100 bg-emerald-50 p-5 flex items-center gap-3 text-emerald-700">
      ...
      <div className="rounded-3xl bg-white border border-slate-100 shadow-lg shadow-slate-900/5 p-6">
        <h4 className="font-bold text-slate-800 mb-4">Personalized care plan</h4>
        ...
        <div className="space-y-3">
          {recs.map((r, i) => (
            ...
            >
              <span className="mt-0.5 grid place-items-center w-7 h-7 shrink-0 rounded-lg bg-brand-100 text-brand-600">
